...
// use \Model\Validator;
class Presenter_API_cLinear_Sscanner_Hot_Comcast extends Presenter_API_cLinear_Template
{
    ...
    public function view()
    {
        ...
        $responseNatTop10 = $this->selectDataByDateRangeAndHot(
            'viper_clinear',                    // DB Connection Label (from app config)
            'NATtop10',                         // DB Table Name (MySQL)
            'downct',                           // Required: The field in the $dbTable that we'll sort on to get the hottest items (i.e. minutes impacted)
        ...
        $responseRegionalTop10 = $this->selectDataByDateRangeAndHot(
            'viper_clinear',                    // DB Connection Label (from app config)
            'regionaltop10',                    // DB Table Name (MySQL)
            'downct',                           // Required: The field in the $dbTable that we'll sort on to get the hottest items (i.e. minutes impacted)
            'date_created',                     // DB Field Name for dates
            $dateStart,                         // The actual start date ingested above
            $dateEnd,                           // The actual end date ingested above
        ...
        $this->response['data'] = array(
                                   'Pillar' => $this->createArrayBucketsByField($responseNatTop10['data'], 'date_created'),
                                   'Super8' => $this->createArrayBucketsByField($responseRegionalTop10['data'], 'date_created'),
                                  );
    }
